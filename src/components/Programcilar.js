...
export default function Programcilar() {
  ...
  const oneCikaninIsmi = () => {
    ...
    const a = programmers.find((programmer) => programmer.id == featuredId);
    return a.isim;
  };
  ...
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featuredId ? "gold" : 'royalblue', // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setFeaturedId(dev.id)}}>Kutla</button>
            </div>
          )
        }
      </div>
